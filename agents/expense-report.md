...
| **K / 11** | 비용 계정 | 명목 (구글시트 E열: 웰메이트/업무활동비/회식비). 시트 없으면 계정과목 분류 | |
...
**명목 시트(구글시트)가 있으면 그것을 우선한다.** CSV(`…/export?format=csv`)를 읽어, 대상 월 구역의 행별 데이터(A=일자, B=가맹점, C=상세, D=금액, E=비고)에서 각 영수증을 **날짜+가맹점**으로 매칭해 **E열 명목**(웰메이트/업무활동비/회식비 등)을 K열에 채운다. 날짜 ±1일·금액 차이는 가맹점+근사금액으로 동일 추정하되 영수증 실제값을 채택하고 비고(S)에 차이를 기록. 매칭 실패는 `[확인필요]`로 보고.

명목 시트가 없을 때만 아래 가맹점/업종 기준 계정과목 규칙으로 분류:
